...
layui.use(['laydate', 'form', 'table', 'util', 'element'], function () {
    ...
    var tableData = table.render({
        elem: '#contentTable'
        , url: '../vesapp/signalquality/list.do' //数据接口
        , page: true //开启分页
        , id: 'tableReload'
        , title: '信号质量报表'
        , response: {
        ...
        , cols: [[ //表头
            {field: 'level', title: '信号强度', sort: true}
            , {
                field: 'time',
                title: '测速时间',
                sort: true,
...
//根据条件查询
    ...
    $("#export").click(function () {
        ...
        $.ajax({
            url: '../vesapp/signalquality/export.do?speedometer=' + speedometer + '&startTime=' + new Date(startTime).getTime() + '&endTime=' + new Date(endTime).getTime() + '&sceneId=' + sceneId + '&areaId=' + areaId,
            type: 'get',
            beforeSend : function(){
                loading("下载中...");
            },
