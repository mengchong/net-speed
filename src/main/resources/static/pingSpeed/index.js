...
layui.use(['laydate', 'form', 'table', 'util'], function () {
    ...
    table.render({
        elem: '#contentTable'
        , url: '../vesapp/pingSpeed/list.do' //数据接口
        , page: true //开启分页
        , id: 'tableReload'
        , response: {
    ...
    $("#export").click(function () {
        ...
        $.ajax({
            url: '../vesapp/pingSpeed/export.do?speedometer=' + speedometer + '&startTime=' + new Date(startTime).getTime() + '&endTime=' + new Date(endTime).getTime() + '&sceneId=' + sceneId + '&areaId=' + areaId,
            type: 'get',
            beforeSend : function(){
                loading("下载中...");
            },
