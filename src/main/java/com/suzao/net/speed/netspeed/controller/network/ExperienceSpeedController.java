...
import com.suzao.net.speed.netspeed.bo.network.ExperienceSpeedListBo;
...
@Api(tags = "体验测速")
@RestController
@RequestMapping({"/vesapp/experienceSpeed"})
public class ExperienceSpeedController {
    ...
    @PostMapping({"/save.do"})
    public JSONObject save(@RequestBody List<ExperienceSpeedBo> experienceSpeedBoList) {

        Date date = new Date();
        ArrayList<ExperienceSpeedBo> experienceSpeedList = new ArrayList<>();

        for (ExperienceSpeedBo experienceSpeedBo : experienceSpeedBoList) {
        ...
        return toSuccess(experienceSpeedApi.insert(experienceSpeedList));
    }

    @ApiOperation(value = "IOS保存")
    @PostMapping({"/saveIOS.do"})
    public JSONObject saveIOS(@RequestBody ExperienceSpeedListBo experienceSpeedBoList) {

        Date date = new Date();
        ArrayList<ExperienceSpeedBo> experienceSpeedList = new ArrayList<>();
        ExperienceSpeedBo experienceSpeedBo;

        for (int i = 0; i < experienceSpeedBoList.getExperienceSpeedBoList().size(); i++) {
            experienceSpeedBo = experienceSpeedBoList.getExperienceSpeedBoList().get(i);
            experienceSpeedBo.setTime(date);
            //判断传输过来的数据是否存在mac属性或mac5G属性
            if (experienceSpeedBo.getMacAddress() != null || experienceSpeedBo.getMacAddress5G() != null) {
                String macAddress = experienceSpeedBo.getMacAddress();
    ...
    public JSONObject listAll(ExperienceSpeedVo experienceSpeedVo) {
        return toSuccess(experienceSpeedApi.selectAll(experienceSpeedVo));
    }
    ...
    public JSONObject list(int pageNumber, int pageSize, String speedometer, Long startTime, Long endTime, String sceneId, String areaId, String apId) {
        ...
        Page<ExperienceSpeedBo, ExperienceSpeedVo> page = new Page<>();
        ExperienceSpeedVo experienceSpeedVo = new ExperienceSpeedVo();
        ...
        experienceSpeedApi.selectPage(page);
    ...
    public JSONObject list(HttpServletResponse response, HttpServletRequest request, String speedometer, Long startTime, Long endTime, String sceneId, String areaId, String apId) throws Exception {
        ...
        List<ExperienceSpeedVo> ExperienceSpeedVoList = experienceSpeedApi.selectAll(experienceSpeedVo);
    ...
    public JSONObject findById(String id) {
        ...
        return toSuccess(experienceSpeedApi.findById(longId));
    }
    ...
    public JSONObject selectPassValue(PassValueVo vo, HttpServletRequest request) {
        List<ExperienceSpeedVo> valueVoList;

        //查询合格标准值
        PassValueBo passValueBo = passValueApi.selectAll();

        vo.setNetworkPassValue(passValueBo.getNetworkPassValue());

        valueVoList = experienceSpeedApi.selectData(vo);
        //根据标识判断查询大于等于/小于还是所有数据
        float speed = 0;
        if (vo.getName() != null && vo.getName().equals("GTET")) {
            for (int i = 0; i < valueVoList.size(); i++) {
                //转换单位
                if (valueVoList.get(i).getSpeed().contains("MB")) {

                    speed = Float.parseFloat(valueVoList.get(i).getSpeed().trim().replace("MB/s", "")) * 1024;
                } else if (valueVoList.get(i).getSpeed().contains("KB")) {
                    speed = Float.parseFloat(valueVoList.get(i).getSpeed().trim().replace("KB/s", ""));

                }
                //如果小于就去除
                if (speed < Float.parseFloat(passValueBo.getNetworkPassValue())) {
                    valueVoList.remove(i);
                }
    ...
    public JSONObject export(List<ExperienceSpeedVo> list, HttpServletRequest request) {
        Map<String, Object> bakMap = new HashMap<>();

        bakMap.put("experienceSpeed", list);
        JSONObject jsonObject = new JSONObject();
...
//        URL=URL.substring(1);
        try {
            JxlsUtils.testExportExcel(URL + "experience.xlsx", bakMap, excelNamePath + newName);
